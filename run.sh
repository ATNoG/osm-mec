...
helm -n osm-mec upgrade --install osm-mec osm-mec-helm-chart --set cfsPortal.deployment.image=localhost:5000/cfs-portal:latest --set meao.deployment.image=localhost:5000/meao:latest --set oss.deployment.image=localhost:5000/oss:latest --set osm.host=$OSM_NBI --set cfsPortal.ossHost=$K8S_DEFAULT_IP
kubectl get pods -A
