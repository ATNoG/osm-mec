#/bin/bash

# Build and push the images
docker build -t localhost:5000/forwarder:latest metrics-forwarder

docker push localhost:5000/forwarder:latest


# Install the new version of the OSM-MEC
helm -n osm-mec upgrade --reuse-values osm-mec deployment/helm-chart \
    --set metricsForwarder.deployment.image=localhost:5000/forwarder:latest \
    --set metricsForwarder.monitoring.deployment.image=localhost:5000/forwarder-monitoring:latest \
    --set metricsForwarder.migration.deployment.image=localhost:5000/forwarder-migration:latest \

# Restart the forwarder pod
kubectl -n osm-mec delete pod -l app=metrics-forwarder
